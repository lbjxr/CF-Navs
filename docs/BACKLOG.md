...
| PROB-11v | 验证欠账 | P2 | 移动端（≤720px）「更多操作」菜单的真机验证：「新增书签 / 新建子分类 / 排序」三项实际只在菜单里出现、首页主分类区不再直显这三个按钮、菜单在视口内不被导航遮挡、触控可点、Esc/外部点击行为一致，且排序会话中的拖拽提示仍可见。jsdom 不应用媒体查询，组件测试证明不了可见性 | PH PROB-11 |
