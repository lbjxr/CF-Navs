...
| ID | 事项 | 说明 |
| --- | --- | --- |
| REL-01 | 完成主项目 v0.4.0 首发 | 维护者已确认（2026-09-12）将原 `v0.2.0` / `v0.3.0` / `v0.4.0` 三批累计成果合并为首次正式发布。以 `develop` 当前安全及验收修复基线为起点，同步版本与变更记录，完成 L0 / L1、推送自动部署及生产 L3 后，再发布 `v0.4.0` 注释 tag 与 GitHub Release；不补发历史 tag、不回滚旧代码、不自动合并 `main`。本轮已获提交、tag、推送、自动部署与隔离验收的明确授权，结果仍须实际验证 |
| REL-02 | 完成 v0.4.0 的 Issue 生命周期闭环 | 在 `v0.4.0` tag 与 GitHub Release 发布、对应验收通过后，分别向 #9、#10、#11、#12、#13 评论该版本及实际验证范围，再手动关闭；验收不足的项保持开放。#15（EdgeOne）继续开放，由 PROB-25 / REQ-12 跟踪；不修改 Project。生产复杂导出样本的 PROB-14 与依赖大版本迁移 PROB-34 不因本次发版或 Issue 关闭而自动完成 |
