...
**FR-4.5 不做**
- 不引入 `--accent-primary` 别名（见 FR-4.1 末条）。
- 不新增任何 accent 相关的用户设置项：`shared/types.ts` 无 accent key，accent 由预设决定是当前的设计意图，加设置项会与 22 个预设的调色形成两套真源。
...
| D-10 | 不新增任何 accent 相关用户设置项 | accent 由 22 个预设决定是现有设计意图，加设置项会形成两套真源（FR-4.5） |
...
FR-2.3（移动端 Action Sheet，现状不触发）；FR-2.4（模块二“不做”边界）；FR-3.4（图标候选无需开发）；FR-3.6（后台边界）；FR-4.4（几何默认值/间距本轮不改，分别由 OQ-7/OQ-6 约束）；FR-4.5（不新增 accent 设置）；FR-5.3 与 FR-5.4（驳回/降级）；FR-5.5（性能护栏，随各阶段验证）；OQ-1 至 OQ-8（待决）。FR-1.6 已并入阶段 2.6，不得遗漏。
