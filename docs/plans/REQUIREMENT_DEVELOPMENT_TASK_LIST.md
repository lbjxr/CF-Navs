...
> **状态：需求已核对，开发未获批。本文只汇总「有需求来源且当前源码确认尚未实现」的功能，不构成开工授权。**
>
> - 核对日期：2026-09-03；核对基线：`develop` 分支当前工作树。
> - **重要前置**：`docs/README.md:15` 明确 `plans/FRONTEND_EXPERIENCE_OPTIMIZATION_REQUIREMENTS.md` 当前是需求评估/决策记录，状态「尚未实现」，**不能直接当作已批准的实现清单**。本文 `REQ-01`~`REQ-11` 全部源自该文档，因此**逐项都需要用户明确批准才可开工**。
> - 本轮未运行 `npm run type-check` / `npm test` / `npm run build` / 浏览器套件，也未执行任何 git、部署或 GitHub 写操作。所有「未实现」结论都是 grep + 逐符号静态核对得出。
...
- 本文新编号统一用 `REQ-NN`，**不占用**既有原生编号（`R-01`~`R-08`、`FR-*`、`OQ-*`、`T*`）。每条在「来源映射」列回指原生编号与文档行号。
...
| 状态 | 条目 |
| --- | --- |
| 已完成 | REQ-08（用户已确认接受视觉变动，逐套按背景调色）|
| 未获批准开工 | REQ-01～REQ-07、REQ-09～REQ-11 |
| 阻塞（等范围澄清） | REQ-12 |
