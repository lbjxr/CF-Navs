...
### PROB-30（P3，已裁定待实现）无预设时 accent 回退仍是旧的共用冷蓝
...
- 裁定：用户 2026-09-05 授权按我的建议执行，即**方案 a：保留回退值，补注释说明它与预设 accent 无关**。三个选项的取舍：
  - 方案 c（让自定义背景也能配 accent）代价与 P3 不匹配：要新增公共设置字段，牵动 `shared/settings.ts` 的归一化与默认值、`SETTINGS_KEYS` / 公开设置白名单、设置面板 UI、服务端校验与既有数据的兼容读取。它还落在 `FR-4.5` 已驳回范围内，需要单独立项而不是顺带做
  - 方案 b（改中性色）是无障碍退步：`--home-accent-color` 是 hover 边框与 focus 环的唯一来源（例如 `HomeCategoryScope.svelte` 的 `.scope-action:focus-visible` 用它做 `box-shadow` 焦点环）。中性灰在浅灰背景与深灰卡片上都难辨识，会削弱键盘焦点可见性；而现在这对冷蓝在任意自定义背景上都还算通用
  - 方案 a 成本最低且不改渲染结果。真实残余风险是：用户把自定义背景也设成同色系蓝时焦点环几乎不可见。这个风险在方案 a 下不消除，只被记录
- 待实现的具体动作（按内容定位，不用行号）：把 `src/lib/appData.ts` 的 `buildHomeBackground` 里 `accentColor` 无预设回退分支的两个字面量提成命名常量（例如 `CUSTOM_BACKGROUND_ACCENT` / `CUSTOM_BACKGROUND_DARK_ACCENT`）并加注释写清三件事——① 它们只在「没有任何预设」时生效；② 与 22 套预设的 accent 无继承关系，改预设不影响它们；③ `#7dd3fc` 与 `ocean-depths` 的 `darkAccent` 取值相同属巧合，不是共享来源，改任一方不必同步另一方。**不改任何渲染结果**
- 残余风险（不在本次范围内消除）：自定义背景与回退 accent 同色系时焦点环对比度不足。真正的解法是方案 c，需要单独立项
- 验证：这是纯常量提取 + 注释，渲染输出不变。跑 `npm run type-check` 与 `npx vitest run` 确认无回归即可，**不新增测试**——没有可观察行为变化，为它写断言只会是钉实现
