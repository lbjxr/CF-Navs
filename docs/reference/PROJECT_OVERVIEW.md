...
- ✅ 自定义背景（浅色/深色主题分别配置纯色/渐变/图片），并内置 13 组毛玻璃渐变与 9 组护眼纯色方案；护眼方案使用综合色相更明确的浅色前台背景和不透明卡片
- ✅ 遮罩颜色与透明度后台可调
- ✅ 卡片背景颜色与透明度后台可调
...
- 毛玻璃方案的书签卡片使用 `backdrop-filter` 透出渐变背景；护眼纯色方案使用不透明卡片和更有层次的低饱和页底，亮暗模式分别维持清晰的文字层级
- 柔和的阴影和过渡动画
- 响应式网格布局
- 移动端优化
