...
- ✅ 两层分类和书签浏览；首页按当前一级分类展示全部分组，并可聚焦二级分类
...
│   ├── HomeCategoryScope.svelte # 当前一级分类标题和二级分类筛选
...
- 首页一次展示一个一级分类，“全部”直接呈现本级书签和所有二级分组，二级分类标签切换聚焦视图；主内容不使用折叠
...
- 首页搜索预计算书签索引；普通浏览只挂载当前一级分类，搜索结果分组使用 `content-visibility: auto` 降低离屏渲染成本
