...
### 三条待裁定项落定（PROB-26 / PROB-04 / PROB-30），仅更新清单未动实现
...
- **PROB-30 方案 a**：保留自定义背景（无预设）时的 accent 回退取值，提成命名常量并注明它与预设 accent 无继承关系。否决另两个选项的理由已记入决策记录——方案 c（让自定义背景也能配 accent）要新增公共设置字段，牵动归一化、白名单、UI、服务端校验与旧数据兼容，代价与 P3 不匹配且落在 `FR-4.5` 已驳回范围；方案 b（改中性色）会削弱键盘焦点可见性，因为 `--home-accent-color` 是 hover 边框与 focus 环的唯一来源。残余风险（自定义背景同色系时焦点环对比度不足）如实记录，不假装被消除。
