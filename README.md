...
前台轻量配置接口 `/api/config` 和公开聚合接口 `/api/public/data` 都使用短 TTL Cloudflare edge cache，并在设置保存或数据导入后失效；公开聚合接口还会复用公开模式鉴权时读取的设置，避免重复读取 D1。后台进入使用 `/api/admin/data` 一次返回分类、书签和完整设置，减少管理端初始化网络往返和 settings 重复读取。后台新增、编辑、删除、排序和保存设置后，前端会用接口返回值增量更新本地 public/admin store，不再额外拉取全量 `/api/public/data`。分类和书签排序查询配有复合索引，降低 D1 排序扫描成本。
